...
void read_gadget_binary_local(char       *filename_root_in,
  ...
  if(flag_filefound){
    ...
    for(i_file=0,n_particles_kept=0;i_file<n_files;i_file++){
      ...
      SID_Bcast(&header,sizeof(gadget_header_info),read_rank);
...
int main(int argc, char *argv[]){
  ...
  for(i_file=i_file_lo;i_file<=i_file_hi;i_file+=i_file_skip){
    ...
    if(n_particles_in_groups>0){
      ...
      x_array      =(REAL *)ADaPS_fetch(plist.data,"x_dark");
      y_array      =(REAL *)ADaPS_fetch(plist.data,"y_dark");
      z_array      =(REAL *)ADaPS_fetch(plist.data,"z_dark");
      vx_array     =(REAL *)ADaPS_fetch(plist.data,"vx_dark");
      vy_array     =(REAL *)ADaPS_fetch(plist.data,"vy_dark");
      vz_array     =(REAL *)ADaPS_fetch(plist.data,"vz_dark");
