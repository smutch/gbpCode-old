...
void seconds2ascii(int n_secs, char *string){
  ...
  if(n_days==0 || n_days>1)
    sprintf(days_plural_string,"s\0");
  else
    sprintf(days_plural_string,"s\0");
  if(n_hrs==0 || n_hrs>1)
    sprintf(hrs_plural_string,"s\0");
  else
    sprintf(hrs_plural_string,"s\0");
  if(n_mins==0 || n_mins>1)
    sprintf(mins_plural_string,"s\0");
  else
    sprintf(mins_plural_string,"s\0");
  if(n_secs==0 || n_secs>1)
    sprintf(secs_plural_string,"s\0");
  else
    sprintf(secs_plural_string,"s\0");
  if(n_days>0)
    sprintf(string,"%d day%s, %d hour%s, %d minute%s and %d second%s",
      n_days,days_plural_string,n_hrs,hrs_plural_string,n_mins,mins_plural_string,n_secs,secs_plural_string);
  else if(n_hrs>0)
