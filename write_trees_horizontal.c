...
void write_trees_horizontal(void  **groups_in, 
      ...
      if(n_groups>0){
        ...
        for(i_group=0,i_subgroup=0;i_group<n_groups;i_group++){
           ...
           if(flag_write_extended){
              ...
              group_file_bridge       =set_match_file( &(groups[i_write%n_wrap][i_group].bridge_backmatch));
           ...
           if(flag_write_extended){
              ...
              SID_fwrite(&group_file_bridge,       sizeof(int),  1,&fp_matches_out);
           ...
           for(j_subgroup=0;j_subgroup<n_subgroups_group[i_write%n_wrap][i_group];j_subgroup++,i_subgroup++){
              ...
              if(flag_write_extended){
                 ...
                 subgroup_file_bridge       =set_match_file( &(subgroups[i_write%n_wrap][i_subgroup].bridge_backmatch));
              ...
              if(flag_write_extended){
                 ...
                 SID_fwrite(&subgroup_file_bridge,       sizeof(int),  1,&fp_matches_out);
   ...
   if(flag_write_allcases){
      ...
      for(i_k_match=0;i_k_match<n_k_match;i_k_match++){
         ...
         if(l_write==0){
            ...
            fprintf(fp,"# (%02d): number of particles in the lost      fragmented back-matched %sgroups's main progenitors\n",i_column++,group_text_prefix);
         ...
         for(i_halo=0;i_halo<n_halos;i_halo++){
            ...
            if(check_mode_for_flag(halos[i_halo].type,TREE_CASE_BRIDGED)){
               ...
               fprintf(fp,"%10.3le %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d\n",
                       a_list[l_write],
                       j_write,
                       i_halo,
                       halos[i_halo].id,
                       ...
                       n_particles_fragmented_lost_main,
            ...
            if(check_mode_for_flag(halos[i_halo].type,TREE_CASE_EMERGED)){
               fp=fp_emerged_out;
               fprintf(fp,"%10.3le %10.3le %8d %8d %8d %8d %8d %8d %8d %10.3f %10.3f %10.3f %8d %8d %8d %8d %8d\n",
                       a_list[l_write],
                       dt_progenitor,
                       j_write,
                       i_halo,
                       halos[i_halo].id,
                       set_match_file(&(halos[i_halo].descendant))-i_write,
                       set_match_file(&(halos[i_halo].bridge_backmatch)),
   ...
   //    the output will be in order of expansion factor in these two cases, opposite
   //    from those above.
