...
void create_ghosts(tree_horizontal_ghost_group_info    **groups_in,
   ...
   for(i_group=0,i_subgroup=0;i_group<n_groups[j_file];i_group++){
      ...
      flag_ghost_merger=check_mode_for_flag(groups_in[i_file%n_wrap][i_group].type,TREE_CASE_MERGER);
      for(i_offset=1,file=i_file+1;i_offset<n_search && file<=i_file_start;i_offset++,file++){
         if(i_offset<group_file_offset){
            ...
            if(flag_progenitor_is_new_ghost){
               // Set the last ghost halo in the chain to be a merger if need-be
