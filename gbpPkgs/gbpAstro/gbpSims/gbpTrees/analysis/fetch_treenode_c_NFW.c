...
double fetch_treenode_c_NFW(tree_info *trees,tree_node_info *halo){
   double c_NFW_halo=1.;
   static double **c_NFW_groups   =NULL;
   static double **c_NFW_subgroups=NULL;
   if(halo!=NULL){
      double **c_NFW;
      if(halo->parent==NULL){
         if(c_NFW_groups==NULL)
            c_NFW_groups=(double **)ADaPS_fetch(trees->data,"c_NFW_groups");
         c_NFW=c_NFW_groups;
      }
      else{
         if(c_NFW_subgroups==NULL)
            c_NFW_subgroups=(double **)ADaPS_fetch(trees->data,"c_NFW_subgroups");
         c_NFW=c_NFW_subgroups;
      }
      double *c_NFW_snap=c_NFW[fetch_treenode_snap_tree(trees,halo)];
      if(c_NFW_snap!=NULL)
