...
void write_treenode_all_hist(tree_info *trees,const char *filename_out_root_in,int i_type,double logM_min,double dlogM,int n_logM){
  ...
  init_treenode_hist(trees,"all","z","Mpeak",TREENODE_HIST_LOG_Y,&hist_all_Mpeak,n_z_bin,logM_min,dlogM,n_logM);
  if(i_type==1){
     ...
     init_treenode_hist(trees,"centrals",    "z","Mpeak",TREENODE_HIST_LOG_Y,&hist_centrals_Mpeak,n_z_bin,logM_min,dlogM,n_logM);
     ...
     init_treenode_hist(trees,"substructure","z","Mpeak",TREENODE_HIST_LOG_Y,&hist_substructure_Mpeak,n_z_bin,logM_min,dlogM,n_logM);
  }
  ...
  SID_Allreduce(SID_IN_PLACE,(hist_all_Mpeak->array),(hist_all_Mpeak->n_x*hist_all_M->n_y),SID_INT,SID_SUM,SID.COMM_WORLD);
  if(i_type==1){
