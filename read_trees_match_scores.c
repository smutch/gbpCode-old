...
void read_trees_match_scores(tree_info *trees,char *filename_SSimPL_dir,int mode){
  ...
  if(check_mode_for_flag(mode,READ_TREES_MATCH_SCORES_GROUPS))
     init_tree_data(trees,(void ***)&match_score_groups_local,sizeof(float),INIT_TREE_DATA_GROUPS,"match_score_groups");
  else
     match_score_groups_local=NULL;
  if(check_mode_for_flag(mode,READ_TREES_MATCH_SCORES_SUBGROUPS))
     init_tree_data(trees,(void ***)&match_score_subgroups_local,sizeof(float),INIT_TREE_DATA_SUBGROUPS,"match_score_subgroups");
  else
