...
int main(int argc, char *argv[]){
  ...
  RNG_info  RNG;
  int       seed=1327621;
  GBPREAL     *x_random=NULL;
  GBPREAL     *y_random=NULL;
  GBPREAL     *z_random=NULL;
  ...
  if(SID.I_am_Master){
    ...
    x_grouping =(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    y_grouping =(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    z_grouping =(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    vx_grouping=(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    vy_grouping=(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    vz_grouping=(GBPREAL  *)SID_malloc(sizeof(GBPREAL)*n_grouping);
    if(SID.n_proc>1){
    ...
    for(i_compute=0;i_compute<4;i_compute++){
      ...
      if(SID.I_am_Master){
        ...
        if(x_interp[0]*x_interp[j_bin-1]<0.){
          ...
          free_interpolate(SID_FARG r_o_interp);
        }
