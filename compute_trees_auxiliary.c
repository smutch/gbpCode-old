...
void compute_trees_auxiliary(char *filename_root,
  ...
  for(k_match=0;k_match<2;k_match++){
    ...
    for(i_write=0;i_write<n_write;i_write++){
      ...
      x_MBP             =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
      y_MBP             =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
      z_MBP             =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
      vx_MBP            =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
      vy_MBP            =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
      vz_MBP            =(REAL   *)SID_malloc(sizeof(REAL)  *n_halos);
