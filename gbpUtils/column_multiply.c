...
*/
    ...
    data_2=(float *)SID_malloc(sizeof(float)*n_data);
    for(i_data=0;i_data<n_data;i_data++){
      data_1[i_data]=random_number(&RNG);
      data_2[i_data]=random_number(&RNG);
    }
    free_RNG(&RNG);
    ...
    float *result_1;
    float *result_2;
    result_1=(float *)SID_malloc(sizeof(float)*n_data);
    result_2=(float *)SID_malloc(sizeof(float)*n_data);
    SID_log("Performing multiplication...",SID_LOG_OPEN|SID_LOG_TIMER);
    for(i_data=0;i_data<n_data;i_data++) result_1[i_data]=data_1[i_data]*data_2[i_data];
    SID_log("Done.",SID_LOG_CLOSE);

    // Perform multiplication
    SID_log("Performing multiplication...",SID_LOG_OPEN|SID_LOG_TIMER);
    calc_array_multiply(data_1,data_2,result_2,n_data,SID_FLOAT,CALC_MODE_DEFAULT);
    SID_log("Done.",SID_LOG_CLOSE);

    // Write Result
    SID_log("Writing results...",SID_LOG_OPEN|SID_LOG_TIMER);
    for(i_data=0;i_data<n_data;i_data++) printf("%f %f -> %f %f\n",data_1[i_data],data_2[i_data],result_1[i_data],result_2[i_data]);
    SID_free(SID_FARG result_1);
    SID_free(SID_FARG result_2);
    SID_log("Done.",SID_LOG_CLOSE);
