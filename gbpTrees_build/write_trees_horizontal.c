...
void write_trees_horizontal(void  **groups_in, 
   ...
   if(flag_write_ghosts){
      ...
      for(i_group=0,i_subgroup=0;i_group<n_groups;i_group++){
         ...
         SID_fwrite(&(groups[i_group].n_subgroups),sizeof(int),1,&fp_trees_out);
      ...
      if(n_groups>0){
        ...
        for(i_group=0,i_subgroup=0;i_group<n_groups;i_group++){
           ...
           if(flag_write_extended || flag_write_pointers){
              ...
              group_snap_forematch =set_match_snapshot(&(groups[i_write%n_wrap][i_group].bridge_forematch));
              group_file_forematch =set_match_file(    &(groups[i_write%n_wrap][i_group].bridge_forematch));
              group_index_forematch=set_match_index(   &(groups[i_write%n_wrap][i_group].bridge_forematch));
           }
           else{
           ...
           for(j_subgroup=0;j_subgroup<n_subgroups_group[i_write%n_wrap][i_group];j_subgroup++,i_subgroup++){
              ...
              if(flag_write_extended || flag_write_pointers){
                 ...
                 subgroup_snap_forematch =set_match_snapshot(&(subgroups[i_write%n_wrap][i_subgroup].bridge_forematch));
                 subgroup_file_forematch =set_match_file(    &(subgroups[i_write%n_wrap][i_subgroup].bridge_forematch));
                 subgroup_index_forematch=set_match_index(   &(subgroups[i_write%n_wrap][i_subgroup].bridge_forematch));
              }
              else{
